...
// GET /api/lessons/worlds?child_id=
...
export async function GET_next(req: NextRequest, childId: string) {
  ...
  // Find first lesson with no progress or in_progress
...
// GET /api/lessons?world_id=&age_band=&child_id=&track=
export async function GET_list(req: NextRequest) {
  ...
  let query = supabase.from('lessons')
    .select('id,world_id,slug,title,description,age_band,track,duration_mins,lesson_number,is_sensitive,is_guided,sort_order,worlds(name,colour)')
    .order('sort_order');

  if (worldId) query = query.eq('world_id', parseInt(worldId));
